...
- **Những gì tôi đã thay đổi trong repo chung:** Xây `app.py` — UI Streamlit tái sử dụng `run_model_tool_loop` từ `chat.py`, hiển thị tool calls/args/results và artifact version; lưu 3 transcript demo (normal, missing-info, action-boundary).
...
- **Nếu làm lại, tôi sẽ cải thiện điều gì:** Tôi sẽ bổ sung trạng thái lỗi provider rõ hơn, hiển thị prompt/tools hash chính xác trong transcript và chuẩn bị thêm transcript cho cancellation và correction nhiều lượt trước khi demo.
...
- [x] `system_prompt.md`, `tools.yaml`, version lxg, runs, eval, transcript, U reposit
.
- [ ] Team has agreed on one repository URL.
- [ ] All members will submit that same URL on VLearn.

**URL repository chung dùng để nộp:**
> URL: Pending team confirmation.
ry report đã  ó trong repository.
- [x] Không có `.env`, API key, token, dữ liệu thật, cache hoặc generated ticket.
- [x] Nhóm trưởng và mọi thành viên đã thống nhất đúng một URL reposit
.
- [ ] Team has agreed on one repository URL.
- [ ] All members will submit that same URL on VLearn.
